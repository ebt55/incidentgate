...
Consequently **no published row names a model or a provider**, and `preliminary.md` says so in a sentence derived from the rows. The evaluation seam that would carry a model column is implemented and exercised end-to-end against a `provider_call` capture in `tests/evaluation/test_model_backed_column.py`; enrolment in `MODEL_BACKED_SCENARIOS` is empty solely because no real capture exists. Obtaining one requires a live provider call, which has not been made.
