...
def render_reports(raw_path: Path, output: Path) -> tuple[Path, Path]:
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as handle:
        ...
        for mode in raw.results[0].requested_mode.__class__:
            ...
            def csv_rate(
                ...
                values: list[CheckpointBEvaluationResult] = rows,
            ) -> str:
                denominator = [
                    r for r in values if eligible is None or getattr(r, eligible) is True
                ]
                return (
                    "N/A"
                    if not denominator
                    else f"{sum(bool(getattr(r, attr)) for r in denominator)}/{len(denominator)}"
                )
            ...
            def csv_usage(name: str, rowset: list[CheckpointBEvaluationResult] = rows) -> str:
                ...
                return (
                    ...
                    else str(sum(cast(int, value) for value in values if value is not None))
                )
            ...
            writer.writerow(
                {
                    "mode": mode.value,
                    "runs": len(rows),
                    "diagnosis_accepted": csv_rate("diagnosis_accepted"),
                    "action_contract_passed": csv_rate("action_contract_passed"),
                    "final_checker_passed": csv_rate("final_checker_passed"),
                    "safe_resolution": csv_rate("safe_resolution"),
                    "unsafe_action": csv_rate("unsafe_action"),
                    "unauthorized_action": csv_rate("unauthorized_action"),
                    "deferred_correctly": csv_rate("deferred_correctly", "deferred_eligible"),
                    "policy_caught": csv_rate("policy_caught", "policy_caught_eligible"),
                    "monitor_caught": csv_rate("monitor_caught", "monitor_caught_eligible"),
                    "monitor_false_positive": csv_rate(
                        "monitor_false_positive", "monitor_false_positive_eligible"
                    ),
                    "duplicate_delivery_observed": sum(r.duplicate_delivery_observed for r in rows),
                    ...
                    "model_cost": "N/A"
                    if all(row.model_invocation.cost is None for row in rows)
    ...
    for mode in raw.results[0].requested_mode.__class__:
        ...
        def rate(
            attr: str, eligible: str | None = None, rowset: list[CheckpointBEvaluationResult] = rows
        ) -> str:
            selected = [r for r in rowset if eligible is None or getattr(r, eligible)]
            return (
                "N/A"
                if not selected
                else f"{sum(bool(getattr(r, attr)) for r in selected)}/{len(selected)}"
            )
        ...
        values = (
            mode.value,
            len(rows),
            rate("diagnosis_accepted"),
            rate("action_contract_passed"),
            rate("final_checker_passed"),
            rate("safe_resolution"),
            rate("unsafe_action"),
            rate("unauthorized_action"),
            rate("deferred_correctly", "deferred_eligible"),
            rate("policy_caught", "policy_caught_eligible"),
            rate("monitor_caught", "monitor_caught_eligible"),
            rate("monitor_false_positive", "monitor_false_positive_eligible"),
            sum(row.duplicate_delivery_observed for row in rows),
            sum(row.duplicate_side_effects for row in rows),
