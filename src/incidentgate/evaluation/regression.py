...
def compare_semantics(
    expected: CheckpointBRawEnvelope, actual: CheckpointBRawEnvelope
) -> RegressionReport:
    ignored = {"latency_ms", "trace_id"}
    expected_rows = {
        (row.scenario_id, row.requested_mode.value, row.trial): row.model_dump(
    ...
    return RegressionReport(
        ...
        mismatches=tuple(map(str, mismatches)),
        expected_semantic_hash=semantic_hash(expected_rows),
        actual_semantic_hash=semantic_hash(actual_rows),
        suite_manifest_digest=expected.suite_manifest_digest,
        git_revision=expected.git_revision,
    )
