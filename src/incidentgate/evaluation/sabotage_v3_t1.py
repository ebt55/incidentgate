...
MODEL_MONITOR_DISCLOSURE: Final[tuple[str, ...]] = (
    ...
    (
        ...
        "`authorization_scope` or `evidence_support_state`, so a verdict here was not copied "
        "from a decision the monitor was shown. That is a claim about the bytes and it is "
        "the whole of it -- it does **not** establish that a verdict follows from the "
        "rationale beside it, and the two can disagree, so read both. It does **not** put a "
