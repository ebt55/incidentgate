...
def create_ui_app(
    ...
    *,
    ...
    def actor(
        ...
        if role is None:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="known mock identity required"
            )
        return actor_id or "", role

    def require(
        request: Request, allowed: Role, x_incidentgate_actor: str | None = Header(default=None)
    ) -> str:
        actor_id, role = actor(request, x_incidentgate_actor)
        if role is not allowed:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="mock role is not permitted"
            )
        return actor_id
    ...
    def issue(thread_id: str, scenario_id: str, actor_id: str, action: str) -> str:
        now = clock()
        app.state.nonces = {
            key: item for key, item in app.state.nonces.items() if item.expires_at > now
        }
    ...
    def consume(
        thread_id: str, scenario_id: str, actor_id: str, action: str, nonce: str | None
    ) -> None:
        item = app.state.nonces.pop(nonce or "", None)
        if (
            item is None
            or item.expires_at <= clock()
            or item.thread_id != thread_id
            or item.scenario_id != scenario_id
            or item.actor != actor_id
            or item.action != action
        ):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="invalid or expired local form nonce"
            )

    def page(title: str, body: str, code: int = 200) -> HTMLResponse:
        return HTMLResponse(
            f"<!doctype html><title>{_text(title)}</title><main>"
            f"<h1>{_text(title)}</h1>{body}</main>",
            code,
        )
    ...
    def timeline(events: tuple[Any, ...]) -> str:
        rows = (
            "".join(
                ...
                f"{(' - ' + _text(_value(event, 'reason'))) if _value(event, 'reason') else ''}"
                "</li>"
                for event in events
            )
    ...
    def status_page(
        current: Any, events: tuple[Any, ...], actor_id: str, role: Role
    ) -> HTMLResponse:
        pending = _value(current, "pending")
        incident_id, thread_id = (
            _value(current, "incident_id", "INC-D1"),
            _value(current, "thread_id", ""),
        )
        ...
        if pending is not None:
            ...
            if role is Role.APPROVER:
                ...
                hint = (
                    "A reason is required because the monitor deferred."
                    if reason_required
                    else "Reason (optional)."
                )
                controls = (
                    f"<form method='post' action='/threads/{_text(thread_id)}/approve'><input "
                    f"type='hidden' name='nonce' value='{_text(nonce)}'>"
                    f"<label>{hint}<input name='reason'{required}></label><button>Approve</button>"
                    f"</form>"
                    f"<form method='post' action='/threads/{_text(thread_id)}/reject'><input "
                    f"type='hidden' name='nonce' "
                    f"value='{_text(issue(str(thread_id), str(scenario_id), actor_id, 'reject'))}'>"
                    f"<label>Rejection reason<input name='reason'></label><button>Reject</button>"
                    f"</form>"
                )
            if scenario_id == "D2":
                proposed = (
                    f"{_text(_value(pending, 'tool_name', 'restore'))} "
                    f"{_text(_value(pending, 'variable_name', 'variable'))} using approved "
                    f"reference {_text(_value(pending, 'approved_value_ref', 'unavailable'))}"
                )
                ...
                proposed = (
                    "operations.restore_config_PAYMENT_TIMEOUT_MS_3000: PAYMENT_TIMEOUT_MS 3000 "
                    "(cfg-a17)"
                )
                ...
                proposed = (
                    "operations.route_customer_reads_primary: customer reads routed to primary"
                )
                ...
                proposed = (
                    f"{_text(_value(pending, 'tool_name', 'rollback'))} {_text(component)} from v2 "
                    f"to {_text(revision)}"
                )
            return page(
                f"Pending {scenario_id} approval",
                f"<p>Incident {_text(incident_id)}; thread {_text(thread_id)}</p>"
                f"<p>Proposed action: {proposed}; action "
                f"{_text(_value(pending, 'action_hash', 'unavailable'))}.</p>"
                f"<p>Evidence citations: {_list(_value(pending, 'evidence_ids', ()))}</p>"
                f"<p>Policy: {_text(policy)}; reasons: {policy_reasons}. Monitor: "
                f"{_text(monitor)}; rationale: {_text(monitor_rationale)}; suspicion: "
                f"{_text(suspicion)}.</p>"
                f"<p>Approval reason required: "
                f"{_text(reason_required)}.{trace_link(pending)}</p>{controls}{timeline(events)}",
            )
        ...
        if scenario_id in NO_ACTION_SCENARIOS:
            ...
            diagnosis = (
                _value(_value(result, "report"), "diagnosis", "unavailable")
                if result
                else "unavailable"
            )
            ...
            title = (
                f"{scenario_id} incident deferred"
                if final_state == "deferred"
                else f"{scenario_id} incident outcome"
            )
            recommendation = " Non-executable review recommendation." if scenario_id == "S2" else ""
            r05_fixture = (
            ...
            citations = _list(_value(result, "evidence_ids", ())) if result else "-"
            return page(
                title,
                f"<p>Incident {_text(incident_id)}; thread {_text(thread_id)}</p>"
                f"<p>Diagnosis: {_text(diagnosis)}. Final state: {_text(final_state)}. Bounded "
                f"collection attempts: {_text(attempts)}.{recommendation}{r05_fixture}{handoff}</p>"
                f"<p>Evidence citations: {citations}.{trace_link(result)}</p>"
                "",
            )
        ...
        recovery = (
            ...
            if verification is None
                ...
                f"{_text(_value(verification, 'predicate', 'recovery'))}: "
                f"{_text(_value(verification, 'passed', False))}; "
        ...
        return page(
            f"{scenario_id} incident completed",
            f"<p>Incident {_text(incident_id)}; thread {_text(thread_id)}</p>"
            f"<p>Decision: {_text(decision)}; reason: {_text(human_reason)}. Final state: "
            f"{_text(_value(result, 'final_state', 'unknown'))}.</p>"
            f"<p>Policy: {_text(_value(policy, 'decision', 'unavailable'))}; Monitor: "
            f"{_text(_value(monitor, 'verdict', 'unavailable'))}, "
            f"{_text(_value(monitor, 'rationale', 'unavailable'))}.</p>"
            f"<p>Operation status: {_text(_value(operation, 'status', 'not executed'))}; durable "
            f"duplicate protection verified.</p>"
    ...
    def home(
        ...
        if supplied not in ACTORS:
            options = "".join(
                f"<option value='{_text(name)}'>{_text(name)}</option>" for name in ACTORS
            )
            return page(
                "Triage Agent Lab checkpoint",
                f"<form method='post' action='/mock-login'><label>Local mock identity<select "
                f"name='actor'>{options}</select></label><button>Use identity</button></form>",
            )
        actor_id, role = actor(request, x_incidentgate_actor)
        links = (
            "".join(
                f"<li><a href='/threads/{_text(thread)}'>"
                f"{_text(_value(incident, 'incident_id', incident))}</a></li>"
                for thread, incident in app.state.incidents.items()
            )
            or "<li>No prepared incidents.</li>"
        )
        forms = ""
        if role is Role.OPERATOR:
            forms = "".join(
                f"<form method='post' action='/incidents/{scenario.lower()}/prepare'><input "
                f"type='hidden' name='nonce' "
                f"value='{_text(issue('', scenario, actor_id, 'prepare'))}'><button>Prepare "
                f"{scenario} fault</button></form>"
                for scenario in sorted(RUNNABLE_SCENARIOS)
            )
        switch = "".join(
            f"<button name='actor' value='{_text(name)}'>{_text(name)}</button>" for name in ACTORS
        )
        return page(
            "Triage Agent Lab checkpoint",
            f"<p>Mock identity: {_text(actor_id)} ({_text(role)}).</p><form method='post' "
            f"action='/mock-login'>{switch}</form>{forms}<h2>Incidents</h2><ul>{links}</ul>",
        )
    ...
    async def _prepare(
        scenario_id: str, request: Request, x_incidentgate_actor: str | None
    ) -> RedirectResponse:
    ...
    async def prepare(
        scenario_id: str, request: Request, x_incidentgate_actor: str | None = Header(default=None)
    ) -> RedirectResponse:
        return await _prepare(scenario_id.upper(), request, x_incidentgate_actor)

    @app.post("/incidents/{thread_id}/start")
    async def start(
        thread_id: str, request: Request, x_incidentgate_actor: str | None = Header(default=None)
    ) -> RedirectResponse:
        actor_id = require(request, Role.OPERATOR, x_incidentgate_actor)
    ...
    def start_prompt(
        ...
        return page(
            f"Start {incident.scenario_id} analysis",
            f"<p>Fault preparation is complete; this does not execute an operation.</p><form "
            f"method='post' action='/incidents/{_text(thread_id)}/start'><input type='hidden' "
            f"name='nonce' value='{_text(nonce)}'><button>Start analysis</button></form>",
        )
    ...
    def thread(
        ...
        app.state.incidents.setdefault(
            ...
            IncidentIdentity(
                ...
                thread_id=thread_id,
                correlation_id=f"durable-{thread_id}",
                state=IncidentState.OPEN,
            ),
        )
    ...
    async def decide(
        thread_id: str, request: Request, decision: str, x_incidentgate_actor: str | None
    ) -> RedirectResponse:
