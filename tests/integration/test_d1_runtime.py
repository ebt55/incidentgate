...
from triage_agent_lab.integration import CheckpointRuntime, PendingApproval
...
def test_start_interrupts_then_recreated_runtime_approves(repository: LabRepository) -> None:
    dsn = repository.dsn
    incident, caller, context = inputs("durable-approval")
    with CheckpointRuntime(dsn) as first:
    ...
    with CheckpointRuntime(dsn) as second:
...
def test_non_approver_cannot_approve_or_mutate(repository: LabRepository) -> None:
...
def test_trace_url_failure_keeps_pending_and_status_available(repository: LabRepository) -> None:
    ...
    try:
        with CheckpointRuntime(repository.dsn, telemetry=telemetry) as runtime:
...
def test_zero_ttl_cannot_issue_or_execute(repository: LabRepository) -> None:
    incident, caller, context = inputs("expired-request")
    with CheckpointRuntime(repository.dsn, approval_ttl=timedelta()) as runtime:
        runtime.start(incident, caller, context)
...
def test_response_loss_retries_from_durable_checkpoint_once(repository: LabRepository) -> None:
    dsn = repository.dsn
    incident, caller, context = inputs("durable-response-loss")
    with CheckpointRuntime(dsn, response_loss_once=True) as first:
