...
def test_d5_approved_cleanup_is_exactly_bounded_and_reject_is_side_effect_free(
    ...
    assert (
        state["log_bytes"] == 32 * 1024 * 1024
        and state["free_bytes"] == 96 * 1024 * 1024
...
def test_d8_fresh_runtime_retry_replays_one_committed_restart(repository: LabRepository) -> None:
    ...
    repository.inject_checkpoint("D8")
    ...
    forged = replay.result.operation.model_copy(
        update={
            "context": replay.result.operation.context.model_copy(
                update={"correlation_id": "forged"}
            )
        }
    )
...
def test_d5_d8_runtime_emits_one_safe_workflow_phase_trace(
...
@pytest.mark.parametrize(
    ...
    (
        ("D5", "bounded simulated-log cleanup (cap 64 MiB)", "approve", 1),
        ("D5", "bounded simulated-log cleanup (cap 64 MiB)", "reject", 0),
        ("D8", "operations.restart api", "approve", 1),
    ),
)
def test_fresh_host_ui_resumes_d5_d8_pending_without_sensitive_action_material(
    ...
    prepared = first.post(
        f"/incidents/{scenario.lower()}/prepare",
        data={"nonce": scenario_nonce},
    ...
    assert (
        first.post(
            f"/incidents/{thread}/start", data={"nonce": _nonce(start.text)}, follow_redirects=False
        ).status_code
        == 303
    )
    ...
    assert all(
        word not in pending.text.lower()
        for word in ("approval token", "token", "secret", "idempotency", "100663296", "33554432")
    )
    assert (
        second.post(
    ...
    assert (
        second.post(
            f"/threads/{thread}/{decision}",
            data={"nonce": _nonce(pending.text, 0 if decision == "approve" else 1)},
            follow_redirects=False,
        ).status_code
        == 303
    )
