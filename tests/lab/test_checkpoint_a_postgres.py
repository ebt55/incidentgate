...
def context(
    ...
    *,
    ...
    return ToolCallContext(incident_id=incident, thread_id=thread,
                           correlation_id=correlation_id or f"corr-{thread}", actor=actor,
...
def citations(
    ...
    *,
    ...
    read = ToolCallContext(incident_id=incident, thread_id=thread,
                           correlation_id=correlation_id or f"corr-{thread}", actor=actor,
...
def action_for(repo: LabRepository, scenario: str, call: ToolCallContext, evidence_ids: tuple[str, ...]) -> CanonicalAction:
    if scenario == "D2":
        return CanonicalAction(tool_name="operations.restore_config", incident_id=call.incident_id,
                               ...
                               arguments={"kind": "restore_config", "component": "api",
                                          "variable_name": "REQUIRED_API_URL",
                                          "approved_value_ref": APPROVED_API_URL_REF})
    return CanonicalAction(tool_name="operations.restart", incident_id=call.incident_id,
                           thread_id=call.thread_id, actor=call.actor, permission=call.permission,
...
def assert_denied_unchanged(repo: LabRepository, call: ToolCallContext, approved: ApprovalToken, scenario: str) -> None:
    with repo._connect() as connection, connection.cursor() as cursor:
        ...
        cursor.execute("SELECT consumed_at FROM approvals WHERE token_id = %s", (approved.token_id,))
...
def test_d2_restores_only_bounded_reference_and_is_idempotent(repository: LabRepository) -> None:
    ...
    action = CanonicalAction(tool_name="operations.restore_config", incident_id="INC-D2", thread_id=call.thread_id,
                             actor=call.actor, permission=call.permission,
                             evidence_ids=citations(repository, "INC-D2", call.thread_id, ("health", "config_diff", "logs")),
                             arguments={"kind": "restore_config", "component": "api", "variable_name": "REQUIRED_API_URL", "approved_value_ref": APPROVED_API_URL_REF})
    ...
    read = ToolCallContext(incident_id="INC-D2", thread_id=call.thread_id, correlation_id=call.correlation_id,
                           actor=call.actor, permission="observability:read")
    fresh = ObservabilityService(repository).get(read, Principal("operator-1", Role.OPERATOR), "config_diff")
    assert fresh.payload["present"] is True and fresh.payload["approved_value_ref"] == APPROVED_API_URL_REF
    assert service.restore_config(call, Principal("operator-1", Role.OPERATOR), action, approved).status.value == "duplicate"
...
def test_d3_response_loss_retries_once_and_reset_is_isolated(repository: LabRepository) -> None:
    ...
    action = CanonicalAction(tool_name="operations.restart", incident_id="INC-D3", thread_id=call.thread_id,
                             actor=call.actor, permission=call.permission,
                             evidence_ids=citations(repository, "INC-D3", call.thread_id, ("health", "db_pool_metrics", "logs")),
                             arguments={"kind": "restart", "component": "api"})
    approved = token(repository, "INC-D3", action)
    service = OperationsService(repository)
    with pytest.raises(ResponseLost):
        service.restart(call, Principal("operator-1", Role.OPERATOR), action, approved, response_loss=True)
    assert service.restart(call, Principal("operator-1", Role.OPERATOR), action, approved).status.value == "duplicate"
    assert repository.checkpoint_state("D3")["mutation_count"] == 1
    read = ToolCallContext(incident_id="INC-D3", thread_id=call.thread_id, correlation_id=call.correlation_id,
                           actor=call.actor, permission="observability:read")
    fresh = ObservabilityService(repository).get(read, Principal("operator-1", Role.OPERATOR), "db_pool_metrics")
...
def test_checkpoint_substitution_cross_thread_and_precondition_fail_safely(repository: LabRepository) -> None:
    ...
    with pytest.raises(ApprovalDenied):
        service.restore_config(call.model_copy(update={"thread_id": "other"}), Principal("operator-1", Role.OPERATOR), action, approved)
    wrong = action.model_copy(update={"arguments": {"kind": "restore_config", "component": "api", "variable_name": "OTHER", "approved_value_ref": APPROVED_API_URL_REF}})
    with pytest.raises(ApprovalDenied):
...
def test_checkpoint_evidence_binding_denies_adversarial_citations(
    ...
    if attack == "foreign_actor":
        evidence_ids = citations(repository, incident, call.thread_id, kinds, correlation_id="corr-action", actor="operator-2")
    elif attack == "foreign_correlation":
        evidence_ids = citations(repository, incident, call.thread_id, kinds, correlation_id="corr-foreign")
    elif attack == "cross_thread":
        evidence_ids = citations(repository, incident, "foreign-thread", kinds, correlation_id="corr-action")
    elif attack == "wrong_incident":
        other = "INC-D3" if scenario == "D2" else "INC-D2"
        other_kinds = ("health", "db_pool_metrics", "logs") if scenario == "D2" else ("health", "config_diff", "logs")
        repository.inject_checkpoint(other.removeprefix("INC-"))
        evidence_ids = citations(repository, other, call.thread_id, other_kinds, correlation_id="corr-action")
    else:
        ...
        with repository._connect() as connection, connection.cursor() as cursor:
            cursor.execute(
                "INSERT INTO immutable_evidence_source (source_id, incident_id, kind, payload, observed_at) "
                "VALUES (%s, %s, 'logs', %s, %s)",
                (source_id, incident, json.dumps({"level": "ERROR", "message": "old observation"}), observed_at),
            )
            cursor.execute(
                "INSERT INTO evidence_records (evidence_id, incident_id, thread_id, correlation_id, tool_name, actor, permission, source_id, observed_at, expires_at, payload) "
                "VALUES (%s, %s, %s, %s, 'observability.logs', %s, 'observability:read', %s, %s, %s, %s)",
                (evidence_id, incident, call.thread_id, call.correlation_id, call.actor, source_id,
                 observed_at, observed_at + timedelta(seconds=120), json.dumps({"level": "ERROR", "message": "old observation"})),
            )
...
def test_checkpoint_action_substitution_and_healthy_precondition_do_not_consume_approval(
    ...
    action = action_for(repository, scenario, call, citations(repository, incident, call.thread_id, kinds))
    approved = token(repository, incident, action)
    operations = OperationsService(repository)
    substituted = action.model_copy(update={"arguments": {"kind": "restart", "component": "other"}})
    with pytest.raises(ApprovalDenied):
        if scenario == "D2":
            operations.restore_config(call, Principal(call.actor, Role.OPERATOR), substituted, approved)
        else:
...
def test_d1_repository_rejects_actor_and_correlation_foreign_evidence(repository: LabRepository) -> None:
    ...
    evidence_ids = citations(repository, "INC-D1", call.thread_id, ("health", "deployment_diff", "logs"),
                             correlation_id="corr-foreign", actor="operator-2")
    action = CanonicalAction(tool_name="operations.rollback", incident_id="INC-D1", thread_id=call.thread_id,
                             actor=call.actor, permission=call.permission, evidence_ids=evidence_ids,
