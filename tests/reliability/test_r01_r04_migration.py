...
def test_011_upgrades_a_001_through_010_journal_once_without_data_loss() -> None:
    ...
    try:
        with psycopg.connect(scoped_dsn) as connection, connection.cursor() as cursor:
            ...
            cursor.execute(
                "INSERT INTO d5_fixture_state (scenario_id, incident_id, component) VALUES ('D5', 'INC-D5', 'api')"
            )
            cursor.execute(
                "INSERT INTO tickets (ticket_id, incident_id, title) VALUES ('upgrade-ticket', 'INC-D5', 'retained')"
            )
            source_id, token_id, one_time_id, operation_key = uuid4(), uuid4(), uuid4(), uuid4()
            cursor.execute(
                "INSERT INTO immutable_evidence_source (source_id, incident_id, kind, payload) VALUES (%s, 'INC-D5', 'health', '{}'::jsonb)",
                (source_id,),
            )
            cursor.execute(
                "INSERT INTO evidence_records (evidence_id, incident_id, thread_id, correlation_id, tool_name, actor, permission, source_id, observed_at, expires_at, payload) VALUES ('upgrade-evidence', 'INC-D5', 'upgrade-thread', 'upgrade-correlation', 'observability.health', 'operator-1', 'observability:read', %s, now(), now() + interval '1 minute', '{}'::jsonb)",
                (source_id,),
            )
            cursor.execute(
                "INSERT INTO approvals (token_id, one_time_use_id, incident_id, action_hash, actor, approver, requested_at, expires_at, approved_at) VALUES (%s, %s, 'INC-D5', %s, 'operator-1', 'approver-1', now(), now() + interval '1 minute', now())",
        ...
        with psycopg.connect(scoped_dsn) as connection, connection.cursor() as cursor:
            ...
            expected_names = [next((ROOT / "db").glob(f"{number:03d}_*.sql")).name for number in range(1, 14)]
            ...
            cursor.execute(
                "SELECT pg_get_constraintdef(oid) FROM pg_constraint WHERE conrelid='immutable_evidence_source'::regclass AND conname='immutable_evidence_source_kind_check'"
            )
        ...
        with psycopg.connect(scoped_dsn) as connection, connection.cursor() as cursor:
            ...
            cursor.execute("SELECT count(*) FROM schema_migrations WHERE name = '011_reliability_r01_r04.sql'")
            assert cursor.fetchone()[0] == 1
            cursor.execute("SELECT count(*) FROM schema_migrations WHERE name = '012_reliability_r05_r08.sql'")
            assert cursor.fetchone()[0] == 1
            cursor.execute("SELECT count(*) FROM schema_migrations WHERE name = '013_reliability_r09_r12.sql'")
            assert cursor.fetchone()[0] == 1
            cursor.execute(
