...
def test_012_upgrades_001_through_011_once_and_retains_prior_durable_rows() -> None:
    ...
    try:
        with psycopg.connect(scoped_dsn) as connection, connection.cursor() as cursor:
            ...
            cursor.execute(
                "INSERT INTO tickets (ticket_id, incident_id, title) VALUES ('r012-retained', "
                "'INC-D1', 'retained')"
            )
        ...
        with psycopg.connect(scoped_dsn) as connection, connection.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT to_regclass(current_schema() || '.r05_collection_runs'), "
                "to_regclass(current_schema() || '.r08_fixture_state')"
            )
            ...
            assert all(
                kind in definition for kind in ("database_locks", "query_plan", "credential_status")
            )
    finally:
        with psycopg.connect(dsn, autocommit=True) as connection, connection.cursor() as cursor:
            cursor.execute(
