...
def action(arguments: dict[str, object] | None = None) -> CanonicalAction:
    return CanonicalAction.model_validate({
        "tool_name": "operations.rollback", "incident_id": "INC-1", "thread_id": "thread-1",
        "actor": "operator-1", "permission": "operations:write", "evidence_ids": ["ev-1"],
        "arguments": arguments or {"kind": "rollback", "component": "api", "target_revision": "v1"},
    })
...
def test_canonical_hash_ignores_action_identity_and_normalizes_evidence_order() -> None:
    first = action({"kind": "rollback", "component": "api", "target_revision": "v1"})
    same_semantics = CanonicalAction.model_validate({**first.model_dump(mode="python"), "action_id": uuid4()})
    second = CanonicalAction.model_validate({
        **first.model_dump(mode="python"), "action_id": uuid4(), "evidence_ids": ["ev-z", "ev-1"],
    })
    third = CanonicalAction.model_validate({
        **first.model_dump(mode="python"), "action_id": uuid4(), "evidence_ids": ["ev-1", "ev-z"],
    })
...
def test_canonical_hash_binds_actor_and_arguments() -> None:
    proposal = action()
    other_actor = CanonicalAction.model_validate({**proposal.model_dump(mode="python"), "actor": "operator-2"})
...
def test_action_rejects_free_form_mutation_payload() -> None:
    with pytest.raises(ValidationError):
        action({"kind": "rollback", "component": "api", "target_revision": "v1", "shell": "rm -rf /"})


def test_action_rejects_duplicate_evidence_and_tool_argument_mismatch() -> None:
    with pytest.raises(ValidationError):
        CanonicalAction.model_validate({
            **action().model_dump(mode="python"), "evidence_ids": ["ev-1", "ev-1"],
        })
    with pytest.raises(ValidationError):
        CanonicalAction.model_validate({
            **action().model_dump(mode="python"), "tool_name": "operations.restart",
        })
...
def test_approval_requires_binding_fields() -> None:
    ...
    request = ApprovalRequest(action_hash=canonical_action_hash(action()), actor="operator-1", one_time_use_id=uuid4(), requested_at=now, expires_at=now + timedelta(minutes=5))
...
def test_approval_token_time_window_and_tool_context_are_frozen() -> None:
    now = datetime.now(UTC)
    request = {
        "action_hash": canonical_action_hash(action()), "actor": "operator-1", "one_time_use_id": uuid4(),
        "requested_at": now, "expires_at": now + timedelta(minutes=5), "approver": "approver-1",
    }
    with pytest.raises(ValidationError):
    ...
    context = ToolCallContext(incident_id="INC-1", thread_id="thread-1", correlation_id="corr-1", actor="operator-1", permission="operations:write", idempotency_key=uuid4())
