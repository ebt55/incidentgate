...
def test_d5_proposer_requires_exact_ordered_fixture_evidence() -> None:
    incident = IncidentIdentity(
        incident_id="INC-D5", scenario_id="D5", thread_id="d5-thread", correlation_id="corr"
    )
    ...
    records = (
        _record(
            "observability.disk_metrics", {"component": "api", "free_bytes": 32 * 1024 * 1024}, 1
        ),
    ...
    _, action = DeterministicD5Proposer().propose(
        incident, Caller(actor="operator-1", role=Role.OPERATOR), context, records
    )
    assert (
        action.evidence_ids == ("ev-1", "ev-2", "ev-3") and action.arguments.max_bytes == 67_108_864
    )
    with pytest.raises(ProposalError, match="missing_required_evidence"):
        DeterministicD5Proposer().propose(
            incident, Caller(actor="operator-1", role=Role.OPERATOR), context, records[:2]
        )


def test_d8_proposal_cites_only_pre_action_health() -> None:
    incident = IncidentIdentity(
        incident_id="INC-D8", scenario_id="D8", thread_id="d8-thread", correlation_id="corr"
    )
    context = ToolCallContext(
        incident_id="INC-D8",
        thread_id="d8-thread",
    ...
    health = EvidenceRecord(
        ...
        observed_at=now,
        expires_at=now + timedelta(minutes=5),
        payload={"component": "api", "status": 503},
    )
    _, action = DeterministicD8Proposer().propose(
        incident, Caller(actor="operator-1", role=Role.OPERATOR), context, (health,)
    )
    ...
    for arguments, reason in (
        ...
        (
            CleanupArgs.model_construct(
                ...
                component="api",
                cleanup_scope="simulated_logs",
                max_bytes=67_108_865,
            ),
            "max_bytes",
        ),
    ):
        action = CanonicalAction.model_construct(
        ...
        assert (
            outcome.decision.value == "deny" and f"argument_constraint:{reason}" in outcome.reasons
        )
    ...
    for scenario in ("d5", "d8"):
        with (
            runtime.start_as_current_span(
                f"{scenario}.workflow", attributes={"incident_id": scenario, "secret": "no"}
            ),
            runtime.start_as_current_span(
                f"{scenario}.approval",
                attributes={"thread_id": f"{scenario}-thread", "raw_log": "no"},
            ),
        ):
            pass
    spans = exporter.get_finished_spans()
    assert all(
        "secret" not in str(span.attributes).lower()
        and "raw_log" not in str(span.attributes).lower()
        for span in spans
    )
    assert (
        spans[0].context.trace_id == spans[1].context.trace_id
        and spans[2].context.trace_id == spans[3].context.trace_id
    )
