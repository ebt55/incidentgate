...
def d1_inputs(label: str) -> tuple[IncidentIdentity, Caller, ToolCallContext]:
    ...
    incident = IncidentIdentity(
        incident_id="INC-D1", scenario_id="D1", thread_id=thread_id,
        correlation_id=f"corr-{thread_id}",
    )
    caller = Caller(actor="operator-1", role=Role.OPERATOR)
    context = ToolCallContext(
        incident_id=incident.incident_id, thread_id=thread_id,
...
def operation_material(
    ...
    context = ToolCallContext(
        ...
        actor="operator-1", permission="operations:write", idempotency_key=uuid4(),
    )
    read_context = context.model_copy(update={"permission": "observability:read", "idempotency_key": None})
    evidence = ObservabilityService(repo).get(
        read_context, Principal("operator-1", Role.OPERATOR), evidence_kind
    )
    action = CanonicalAction(
        tool_name="operations.rollback", incident_id="INC-D1", thread_id=thread_id,
        actor="operator-1", permission="operations:write", evidence_ids=(evidence.evidence_id,),
        arguments=RollbackArgs(kind="rollback", component="api", target_revision="v1"),
    )
    now = datetime.now(UTC)
    token = ApprovalToken(
        action_hash=canonical_action_hash(action), actor="operator-1", approver="approver-1",
        one_time_use_id=uuid4(), requested_at=now, approved_at=now,
...
def test_invariant_1_no_mutation_without_persisted_bound_approval(repository: LabRepository) -> None:
    ...
    with pytest.raises(ApprovalDenied, match="approval is missing"):
        OperationsService(repository).rollback(direct_context, Principal("operator-1", Role.OPERATOR), action, token)
    repository.record_approval(token, "INC-D1")
    substituted = action.model_copy(update={"thread_id": "different-thread"})
    with pytest.raises(ApprovalDenied):
        OperationsService(repository).rollback(direct_context, Principal("operator-1", Role.OPERATOR), substituted, token)
    assert_initial(repository)
...
def test_invariant_3_actions_cite_fresh_same_thread_observability_evidence(repository: LabRepository) -> None:
    ...
    with repository._connect() as connection, connection.cursor() as cursor:
        cursor.execute(
            "SELECT evidence_id, incident_id, thread_id, permission, expires_at FROM evidence_records WHERE evidence_id = ANY(%s)",
            (list(pending.evidence_ids),),
        )
        rows = cursor.fetchall()
    assert {row["evidence_id"] for row in rows} == set(pending.evidence_ids)
    assert all(row["incident_id"] == "INC-D1" and row["thread_id"] == incident.thread_id for row in rows)
    assert all(row["permission"] == "observability:read" and row["expires_at"] > datetime.now(UTC) for row in rows)

    context_a, action_a, _ = operation_material(repository, "same-thread")
    other_read = ToolCallContext(incident_id="INC-D1", thread_id="other-thread", correlation_id="corr-other", actor="operator-1", permission="observability:read")
    foreign = ObservabilityService(repository).get(other_read, Principal("operator-1", Role.OPERATOR), "health")
    cross_thread = action_a.model_copy(update={"evidence_ids": (foreign.evidence_id,)})
    now = datetime.now(UTC)
    token = ApprovalToken(action_hash=canonical_action_hash(cross_thread), actor="operator-1", approver="approver-1", one_time_use_id=uuid4(), requested_at=now, approved_at=now, expires_at=now + timedelta(minutes=5))
    repository.record_approval(token, "INC-D1")
    with pytest.raises(ApprovalDenied, match="evidence"):
        OperationsService(repository).rollback(context_a, Principal("operator-1", Role.OPERATOR), cross_thread, token)
    assert_initial(repository)
...
def test_invariant_4_untrusted_tool_text_cannot_change_d1_authority(repository: LabRepository) -> None:
    ...
    with repository._connect() as connection, connection.cursor() as cursor:
        ...
        rows = cursor.fetchall()
    assert all(row["actor"] == "operator-1" and row["permission"] == "observability:read" for row in rows)
    hostile_rows = [
        row for row in rows
        if row["tool_name"] == "observability.logs" and row["payload"] == {"message": hostile, "level": "ERROR"}
    ]
...
def test_invariant_5_d1_control_failures_are_explicit_safe_states() -> None:
    """D1 exposes explicit auth, validation, and stale-evidence failures; timeout state is later-scenario scope."""
    ...
    try:
        read_context = ToolCallContext(incident_id="INC-D1", thread_id="denied", correlation_id="corr-denied", actor="operator-1", permission="observability:read")
        with pytest.raises(PermissionDenied):
            ObservabilityService(repository).get(read_context, Principal("other", Role.OBSERVER), "health")
        with repository._connect() as connection, connection.cursor() as cursor:
            ...
            cursor.execute(
                "INSERT INTO immutable_evidence_source (source_id, incident_id, kind, payload, observed_at) VALUES (%s, %s, %s, %s, %s)",
                (
                    uuid4(),
                    "INC-D1",
                    "deployment_diff",
...
def test_invariant_7_d1_tool_records_have_complete_bound_context(repository: LabRepository) -> None:
    ...
    assert status.result is not None and status.result.operation is not None
    with repository._connect() as connection, connection.cursor() as cursor:
        cursor.execute("SELECT incident_id, thread_id, correlation_id, actor, permission FROM evidence_records WHERE thread_id = %s", (incident.thread_id,))
        evidence_rows = cursor.fetchall()
        cursor.execute("SELECT incident_id, thread_id, correlation_id, actor, permission, idempotency_key FROM operation_ledger WHERE thread_id = %s", (incident.thread_id,))
        ledger = cursor.fetchone()
    assert len(evidence_rows) == 4
    assert all(row == {"incident_id": "INC-D1", "thread_id": incident.thread_id, "correlation_id": incident.correlation_id, "actor": "operator-1", "permission": "observability:read"} for row in evidence_rows)
    assert ledger == {"incident_id": "INC-D1", "thread_id": incident.thread_id, "correlation_id": incident.correlation_id, "actor": "operator-1", "permission": "operations:write", "idempotency_key": status.result.idempotency_key}
    with pytest.raises(ValidationError):
