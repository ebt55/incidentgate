...
def test_planned_matrix_is_explicit_with_r01_r08_implemented() -> None:
    ...
    checkpoints, planned, sabotage = (
        matrix["checkpoint_contracts"],
        matrix["planned_additional"],
        matrix["sabotage_tier"],
    )
    ...
    assert (
        matrix["full_suite_total"]
        == matrix["checkpoint_total"] + matrix["planned_additional_total"]
    )
    ...
    assert {
        item["id"] for item in planned if item["runtime_status"] == "implemented"
    } == implemented
    assert all(
        item["runtime_status"] == "planned" for item in planned if item["id"] not in implemented
    )
